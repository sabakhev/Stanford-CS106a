...
/** time of delay */
    ...
    private int PADDLE_CHANGING_WIDTH =  PADDLE_WIDTH;
...
/** Runs the Breakout program. */
	...
	// this method allows paddle to move along x and y directions
	public void mouseMoved (MouseEvent e) {
		if (e.getY()<HEIGHT/2) {
			paddle.setLocation(e.getX()-PADDLE_CHANGING_WIDTH/2,HEIGHT/2-PADDLE_HEIGHT/2);
		} else if (e.getY()>HEIGHT -PADDLE_Y_OFFSET) {
			paddle.setLocation(e.getX()-PADDLE_CHANGING_WIDTH/2,HEIGHT -PADDLE_Y_OFFSET-PADDLE_HEIGHT/2);
		} else 
		paddle.setLocation(e.getX()-PADDLE_CHANGING_WIDTH/2, e.getY()-PADDLE_HEIGHT/2);
	}
	...
	private void defineMovementOfTheBall() {
		while(ball.getY()+2*BALL_RADIUS+vy<HEIGHT&NTURNS!=0&NUM_BLOCKS!=0) {
			ball.move(vx, vy);
			pause(DELAY);
			if(NUM_BLOCKS == 3*NBRICKS_PER_ROW*NBRICK_ROWS/4) {
				PADDLE_CHANGING_WIDTH=3*PADDLE_WIDTH/4;
			} 
			if(NUM_BLOCKS==NBRICKS_PER_ROW*NBRICK_ROWS/2) {
				PADDLE_CHANGING_WIDTH=PADDLE_WIDTH/2; 
			} 
			paddle.setSize(PADDLE_CHANGING_WIDTH,PADDLE_HEIGHT);
			defineCollision();
			defineWinning();
		}
	...
	// this method defines three types of collisions
	...
	//		bounceClip.play();
		...
		if (collider2 == paddle) {
			vy=-vy;
			if (X_CENTRE<paddle.getX()+PADDLE_CHANGING_WIDTH/2&vx>0) {
				vx=-vx;
			}
			if (X_CENTRE>paddle.getX()+PADDLE_CHANGING_WIDTH/2&vx<0) {
				vx=-vx;
	...
	// this method changes speed  when indicated
