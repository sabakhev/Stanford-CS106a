...
// *The main extended part is in the ExtendedFacePamphletCanvas.java class.
// *I have added new information for each profile : mutual friends or friends of friends. 
// *When somebody's profile is looked up, people in common graph is shown and also number along with number of mutual friends.

public class ExtendedFacePamphlet extends Program implements FacePamphletConstants {
	...
	private ExtendedFacePamphletCanvas canvas;
	...
	public void init() {
		...
		addComponentsNorth();
		addComponentsWest();
		canvas = new ExtendedFacePamphletCanvas();
		add(canvas);
	}
	...
	private void addingProfile() {
		...
		if (!isItEmpty(typedName)) {
			if (database.containsProfile(typedName)) {
				canvas.displayProfile(database.getProfile(typedName), database);
				...
				canvas.displayProfile(newProfile, database);
	...
	private void updatingProfileStatus() {
		...
		if (!isItEmpty(newStatus)) {
			if (currentProfile != null) {
				...
				canvas.displayProfile(database.getProfile(currentProfile.getName()), database);
